...
if (!class_exists('wp_pluginbase_v2_3')) {

    abstract class wp_pluginbase_v2_3 {
                ...
                case 'license':
                    ...
                    if ($expires !== false) {
                        if (strtotime($expires) < strtotime(date("Y-m-d"))) {
                            $valid = 'invalid'; //it has expired!
                        }
                    }

                    echo '<input class="regular-text license-input ' . ($valid !== false ? $valid:'') . ' ' . $class . '" type="password" id="' . $id . '" name="' . $this->plugin_slug . '[' . $id . ']" value="' . $license . '" />';
                    echo '<input class="license-check-button" type="button" name="license_button" value="' . __('Validate', $this->plugin_slug) . '" />';
                    echo '<span style="display:none" class="license-nonce">' . wp_create_nonce($this->plugin_slug . '_ajax-license-nonce') . '</span>';
...
//            }
        ...
        function admin_settings_render_page() {
            ...
            if (file_exists($this->plugin_dir . "settings.php")) {
                ...
                require_once($this->plugin_dir . "includes/wp_pluginbase_v2_3_default_settings.php");

                $settings_summary = $this->apply_filters('settings_summary', '');
                $settings_title = $this->get_settings_title();

                wp_pluginbase_v2_3_default_settings::render($settings_title, $settings_summary, $this->plugin_slug, $this->_settings_tabs);
            }
        }
